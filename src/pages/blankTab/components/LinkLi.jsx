...
export default function Linkli({ prefix, value, url, editMode, handleRemoveLink }) {
  return (
      <li className='inline-block font-tempesta mx-3 my-1 p-0 text-left transition-all'>
          {
            !editMode ? (
              ...
              <Link className='cursor-pointer decoration-transparent' onPress={() => { handleRemoveLink({ url }) }} >
                <span className='w-12 p-3 text-center text-black rounded-s-md bg-red-400 :hover:bg-red-800'>x</span>
              </Link>
            )
          }
